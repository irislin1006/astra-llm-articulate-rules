"""
Step 3: Articulation Faithfulness Test

Tests if the rules articulated in Step 2 faithfully explain the classification behavior from Step 1.

Method:
1. Load articulated rules from Step 2 results
2. Create prompts with explicit rules
3. Classify same test set using explicit rules
4. Compare accuracy with Step 1 few-shot baseline
"""
...
def run_articulation_faithfulness(
    model_name: str,
    datasets: Dict,
    ...
    Args:
        ...
        datasets: Dictionary of task datasets
    ...
    for task_id, dataset in tqdm(datasets.items(), desc=model_name):
        ...
        for example in tqdm(test_examples, desc=f"Rule-based {task_id}", leave=False):
            ...
            prediction_text = response['response'].strip()
...
def main():
    ...
    for model_name in models:
        ...
        rule_based_results = run_articulation_faithfulness(model_name, datasets, output_dir)
